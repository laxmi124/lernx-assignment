...
let sideBarIcons = [
  { icon: <Squares2X2Icon />, text: "Home" },
  { icon: <BriefcaseIcon />, text: "Job Invites" },
  { icon: <ArrowTrendingDownIcon />, text: "Analytics" },
  { icon: <UserIcon />, text: "My Profile" },
  { icon: <CodeBracketIcon />, text: "Explore" },
];

let feedSctionDetails = [
  {
    url: demoImage1,
    text: "Indonesia UI Designer",
  },
  {
    url: demoImage2,
    text: "Prototyping Club",
  },
];
...
const SideBar = () => {
    ...
    return (
      <div className=" my-4 border-b border-gray-300 pb-4">
        {sideBarIcons.map((element) => (
          <div
            key={element?.icon}
            className="flex mb-2 justify-start items-center gap-3 pl-5 hover:bg-slate-300 p-2 rounded-md group cursor-pointer hover:shadow-lg m-auto"
          >
            <div className="h-7 w-7 text-indigo-500 group-hover:text-white ">
              {element?.icon}
            </div>
  ...
  const SideBarCommunitiesFeedSection = () => {
    return (
      <div className="flex flex-col justify-start item-center">
        ...
        <div className=" my-4 pb-4">
          {feedSctionDetails.map((feedDetails) => (
            <div
              key={feedDetails?.text}
              className="flex mb-2 justify-start items-center gap-3 pl-5 hover:bg-gray-900 p-2 rounded-md group cursor-pointer hover:shadow-lg m-auto"
            >
              <ProfileImage url={feedDetails?.url} />
